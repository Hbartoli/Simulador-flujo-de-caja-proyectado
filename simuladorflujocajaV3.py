...
st.markdown("Proyectá tu caja controlando picos estacionales de ventas y monitoreando el punto más crítico de tu liquidez.")
...
with col_config1:
    ...
    st.caption("Ajustá el rendimiento esperado de cada mes (100% = Normal, 50% = Caída, 150% = Temporada Alta).")
    ...
    meses_unicos = list(dict.fromkeys(meses_indices))
...
with col_config2:
    ...
    st.caption("Configuración estricta de columnas para evitar errores de filas vacías (None).")
...
# Procesar tabla limpiando nulos de forma segura
dict_gastos_por_dia = {}
...
# --- NUEVO CÁLCULO: SALDO MÍNIMO HISTÓRICO ---
indice_minimo = df_diario["Saldo Efectivo"].idxmin()
saldo_minimo_valor = df_diario["Saldo Efectivo"].iloc[indice_minimo]
fecha_saldo_minimo = df_diario["Fecha"].iloc[indice_minimo]

# --- SECCIÓN: MÉTRICAS CLAVE ACTUALIZADAS ---
st.markdown("---")
col1, col2, col3, col4 = st.columns(4)
with col1:
    st.metric(label="Ingreso Diario Promedio", value=f"${df_diario['Ingresos'].mean():,.2f}")
with col2:
    st.metric(label="Egreso Diario Promedio", value=f"${df_diario['Egresos Totales'].mean():,.2f}")
with col3:
    # Mostrar el punto más bajo de la caja proyectada
    st.metric(
        label="📉 Saldo Mínimo Histórico", 
        value=f"${saldo_minimo_valor:,.2f}",
        delta=f"Fecha: {fecha_saldo_minimo.strftime('%d/%m/%Y')}",
        delta_color="inverse" if saldo_minimo_valor < (efectivo_inicial * 0.2) else "normal"
    )
with col4:
    if fecha_quiebra:
        st.error(f"🚨 Runway Agotado el: {fecha_quiebra.strftime('%d/%m/%Y')}")
    else:
        st.success("✅ Caja Saludable en todo el período.")
...
with col_graf1:
    ...
    # Marcador visual para el punto de saldo mínimo histórico
    fig_linea.add_trace(go.Scatter(
        x=[fecha_saldo_minimo], y=[saldo_minimo_valor], mode='markers', name='Punto Más Crítico',
...
with col_graf2:
    ...
    fig_barra.update_layout(title="Comparativa Mensual Consolidada", 
...
with col_torta:
    df_torta = pd.DataFrame({"Categoría": 
                             list(gastos_operativos_dict.keys()), "Costo Diario": 
                             list(gastos_operativos_dict.values())})
...
with col_info_torta:
    st.markdown("#### 💡 Análisis de Estrategia Financiera")
    st.warning(f"Atención con el punto crítico: Tu nivel de reserva más bajo llegará a ${saldo_minimo_valor:,.2f} el día {fecha_saldo_minimo.strftime('%d/%m/%Y')}.")
    st.markdown("""
    Esta métrica te permite planificar financiamiento antes de llegar a esa fecha o renegociar los plazos de los gastos extraordinarios que cargaste a la derecha para suavizar la curva de efectivo."""
    )
...
nombre_archivo = f"cashflow_analisis_critico_{datetime.today().strftime('%Y%m%d')}.xlsx"
st.download_button(label="📥 Descargar Reporte Financiero Completo en Excel (.xlsx)",
                   data=excel_data, 
